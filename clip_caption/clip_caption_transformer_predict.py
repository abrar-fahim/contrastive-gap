...
from src.config import clip_caption_transformer_model_weight_paths as WEIGHTS_PATHS
...
class Predictor(cog.BasePredictor):
    ...
    def predict(self, 
                ...
                use_beam_search: bool = Input(default=False,
        ...
        with torch.no_grad():
            ...
            print("prefix embed shape ", prefix_embed.shape)
            # clip_project accepts batches of prefixes


        if use_beam_search:

            generated_captions = [generate_beam(model, self.tokenizer, embed=torch.unsqueeze(prefix_embed[i], 0))[0] for i in range(preprocessed_images.shape[0])]
            ...
            generated_captions = [generate2(model, self.tokenizer, embed=torch.unsqueeze(prefix_embed[i], 0)) for i in range(preprocessed_images.shape[0])]
